            ...
            <ul>
                {props.tasks.map((el) => {
                    const onChangeCheckboxHandler = (e: ChangeEvent<HTMLInputElement>) => {
                        props.taskStatusChanger(el.id, e.currentTarget.checked, props.todolistId)
                    }
                    const onTaskTitleChangeHandler = (newTitle: string) => {
                        props.taskTitleChanger(el.id, newTitle, props.todolistId)
                    }
                    return (
                        <div key={el.id} className={el.isDone ? 'is-done' : ''}>
                            <IconButton aria-label="delete"
                                        onClick={() => props.taskRemover(el.id, props.todolistId)}
                                        size="small">
                                <Delete fontSize="small"/>
                            </IconButton>
                            <input
